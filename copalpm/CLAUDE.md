...
│   ├── project_lookup.py  # `whose <path>` — given any file or folder, report which project owns it
│   ├── setup_cmd.py       # `setup` / `teardown` orchestration (service + shell install in one step)
...
└── tests/
    ...
    │   └── test_project_lookup.py  # find_project_for_path + `whose` CLI handler
    └── integration/
...
copalpm whose <path> [--json]      # which project owns this file or folder?
...
## Path → project lookup (`whose`)

`copalpm whose <path>` answers: "which registered project owns this file or
folder?" The same primitive (`project_lookup.find_project_for_path`) is the
intended back-end for future file-targeted shell verbs and editor extensions —
external callers should go through `copalpm whose --json <path>` over
subprocess rather than reinventing the walk.

Hybrid two-pass algorithm:

1. **Registry-prefix (fast path, zero filesystem stats).** Normalize the
   target and every registry root via `_norm` (`Path.resolve(strict=False)` +
   `os.path.normcase`) and pick the **deepest** registry root whose
   normalized form is an ancestor of the target. The match is anchored at
   `os.sep` so `C:\Work\Alpha` does not spuriously match `C:\Work\AlphaBeta`.
   Nested projects naturally return the inner one without extra policy code.
2. **Walk-up fallback.** Only on a Pass-1 miss. Walks parents looking for
   ...
   downstream callers like "start timer for X" would fail. Pass-2 hits set
   `drift=True` because the registry's stored path is, by definition, stale.

Returns `None` for: target outside all roots with no walkable `project.yaml`,
a walkable `project.yaml` whose id is unregistered, malformed YAML, or
permission errors during the walk. Non-existent target paths still work for
Pass 1 (`.resolve(strict=False)` is tolerant). UNC ↔ mapped-drive aliasing on
...
- 21 unit tests for `project_lookup` — `_is_under` prefix-anchor semantics, Pass-1 registry-prefix matches (file inside, folder is root, nested deepest-wins, non-existent target), misses (outside any root, unregistered yaml, empty registry), Pass-2 drift recovery (renamed folder, malformed YAML during walk, idless YAML), platform-gated case sensitivity, and the `cmd_whose` CLI handler (human + JSON shapes, exit codes, drift marker)
