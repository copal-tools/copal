...
> Last updated: 2026-05-14 (Decimal serialization hotfix + phase 6 F4 wrap-up).
...
| Sec-A + Perf-A (2026-05-13 → deployed 2026-05-14) | Second security/perf audit. Server: 50 MB Content-Length body limit (was 10 MB; an earlier `_receive`-wrapping streaming counter was reverted on 2026-05-14 — see "Middleware hotfix" below); `X-Confirm-Delete` required on `DELETE /projects` + `/admin/cleanup-orphans`; identity headers validated against `[\w.@-]{1,64}`; `ETag`+`If-None-Match` on `GET /projects` short-circuits the dashboard's 60 s poll. Client: atomic downloads via `.partial` + `os.replace` (no half-written files); HTTP pool sized to 32 (env `COPAL_HTTP_POOL`); per-chunk 120 s read timeout; SHA-256 chunk size unified to 1 MiB. Persistent `.copal/hash_cache.json` keyed by `(rel_path, mtime_ns, size)` — second pull of a 50 GB project re-uses cached hashes; conflict re-hash reuses the scan's already-computed values (eliminates the synchronous re-hash loop). Path-traversal guard now calls `realpath` against a resolved safe root (closes symlink/junction bypass). `.copalignore` precompiled (set + regex-union). Progress callbacks throttled to ≤ 1 flush/100 ms. New backup suffix uses `time.time_ns()`. 20 new unit tests (90 unit total), 4 new integration tests (35 integration total). All 35 integration tests pass against the live server after deploy. Audit report appended to [SECURITY_AUDIT_REPORT.md](../SECURITY_AUDIT_REPORT.md). | server/app/main.py, client/copal_core/{transport,sync,fs,api}.py, client/tui.py, client/tests/ |
| Middleware hotfix (2026-05-14) | Initial Sec-A.3 deploy crashed every `GET /projects` with a 500 because the new body-size middleware (a) wrapped `request._receive` to count bytes — which Starlette `BaseHTTPMiddleware` doesn't propagate into `call_next`'s downstream request — and (b) had a `return` inside its `finally` block (Python 3.14 `SyntaxWarning: 'return' in a 'finally' block`) that suppressed downstream exceptions and turned them into opaque 500s. Reverted to a Content-Length-only check; kept the 50 MB cap. Lesson: don't touch private Starlette attributes (`_receive`) inside `BaseHTTPMiddleware`; never put `return` in `finally`. Smoke-test the server end-to-end (boot + `GET /projects`) before declaring a middleware deploy done. | server/app/main.py |
| License audit (2026-05-14) | Verified every direct + sampled transitive dep against Apache 2.0 compatibility: no GPL/AGPL/SSPL/BSL anywhere. MPL-2.0 deps (pathspec, certifi) and LGPL-3.0 dep (psycopg2-binary) are library-consumed without modification, so neither triggers source-disclosure obligations. Captured in [THIRD_PARTY_NOTICES.md](../THIRD_PARTY_NOTICES.md) at the umbrella level with reproducibility commands. Result: safe to release publicly. | THIRD_PARTY_NOTICES.md, .gitignore |
| Decimal serialization hotfix (2026-05-14) | `GET /projects` was 500ing with `TypeError: Object of type Decimal is not JSON serializable` on every request — the middleware hotfix above had cleared the noise, exposing a latent bug present since the F1/Sec-A deploy. Root cause: PostgreSQL `SUM(BIGINT)` returns `NUMERIC` → SQLAlchemy → `decimal.Decimal`; the endpoint serializes via `Response(content=json.dumps(payload))` which bypasses FastAPI's encoder and stdlib `json.dumps` can't handle `Decimal`. `/server/stats` doesn't trip it because it returns a plain `dict` and FastAPI's encoder handles `Decimal`. Fix: explicit `int()` casts on `total_storage_bytes`, `version_count`, `author_count` at the serialization boundary in `list_projects`. Adds `TestProjectsListSerialization` (2 cases) — locks the regression. Captured as gotcha #12. | server/app/main.py, client/tests/integration/test_server.py |
...
12. **Don't return SQL aggregate results through `Response(content=json.dumps(...))`.** PostgreSQL `SUM(BIGINT)` returns `NUMERIC`, which SQLAlchemy maps to Python's `decimal.Decimal`. Stdlib `json.dumps()` can't serialize `Decimal` and raises `TypeError: Object of type Decimal is not JSON serializable` — failing the entire response with an opaque 500. FastAPI's encoder *does* handle `Decimal` (encodes as float), so returning a plain `dict`/`list` works (`/server/stats` does this). When you need the explicit `Response` path — e.g. to attach `ETag`/`Cache-Control` headers (`/projects` does this) — coerce numeric aggregates to plain Python types at the serialization boundary (`int(r[7]) if r[7] is not None else 0`). Bit us once on `GET /projects` post-2026-05-13 (`SUM(size_bytes)` for `total_storage_bytes`); fixed in `957b991` / hotfix PR #2.
